...
const StringArraySchema = zod_1.z.array(zod_1.z.string().max(100)).max(10);
const FindPartnerPayloadSchema = zod_1.z.object({
    chatType: zod_1.z.enum(['text', 'video']),
    interests: StringArraySchema,
});
const RoomIdPayloadSchema = zod_1.z.object({
    roomId: zod_1.z.string().regex(/^[a-zA-Z0-9#-_]+$/).max(100),
});
...
const WebRTCSignalPayloadSchema = RoomIdPayloadSchema.extend({
    signalData: zod_1.z.any(),
});
function removeFromWaitingLists(socketId) {
...
io.on('connection', (socket) => {
    ...
    socket.on('findPartner', (payload) => {
        try {
            ...
            const currentUser = { id: socket.id, interests, chatType };
                ...
                if (userIndexInRoom > -1) {
                    ...
                    if (partnerId) {
                        ...
                        if (partnerSocket)
                            ...
                            console.warn(`[CLEANUP_USER_WARN] Partner socket ${partnerId} not found to explicitly leave room ${room.id}.`);
                    }
